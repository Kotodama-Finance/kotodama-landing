...
| **Link rot en /method/** | **3 rotos confirmados con dos instrumentos (2026-08-06), pendiente de decisión del autor** — la regla de esa página es que las URLs las aprueba él: Zenginkyo `/en/stats/` → 404 (la vigente es `/en/statistics/`); `giaj.or.jp` y `liaj.or.jp` → NXDOMAIN (las vigentes: `sonpo.or.jp/en/` y `seiho.or.jp/english/`, verificadas cargando y confirmando la institución). Daiwa (`dir.co.jp`) rechaza conexiones desde dos puntos de vista distintos — probable geo-bloqueo, verificar a mano desde Japón, no es rot confirmado. |
